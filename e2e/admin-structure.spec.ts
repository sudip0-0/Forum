...
test.describe("Admin structure management", () => {
  ...
  test("structure manager loads", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("Structure Manager", { timeout: 10000 });
  ...
  test("user management shows user list", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("Manage Users", { timeout: 10000 });
  ...
  test("moderation history page loads", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toContainText("Moderation History", { timeout: 10000 });
  });

  test("suspension requires a reason", async ({ page }) => {
    await page.goto("/admin/users");
    const input = page.getByTestId("suspension-reason-member5");
